...
Employee* addEmployee(Employee* employees, int &numEmployees, int &largestId, int salary, int department) {
    largestId++;
    ...
    extendedEmployees[numEmployees].setId(largestId);
...
void processOperations(const string &operationsFilename, Employee* &employees, int &numEmployees, int &largestId) {
    ...
    while (getline(file, line)) {
        ...
        if (operation == "ADD") {
            ...
            employees = addEmployee(employees, numEmployees, largestId, stoi(salary), stoi(department));
        } 
